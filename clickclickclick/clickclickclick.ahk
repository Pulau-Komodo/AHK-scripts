...
ChangeInterval(ItemName, ItemPos, MyMenu) {
    ...
    if ItemName == "Change plain interval" {
        NewValue := InputBox("Change the interval between clicks when not using any modifiers.`nTime in ms:",
            "Plain interval", , PlainInterval)
    } else if ItemName == "Change Ctrl interval" {
        NewValue := InputBox("Change the interval between clicks when holding down Ctrl.`nTime in ms:", "Ctrl interval", ,
            CtrlInterval)
    } else if ItemName == "Change Shift interval" {
        NewValue := InputBox("Change the interval between clicks when holding down Shift.`nTime in ms:",
            "Shift interval", , ShiftInterval)
    }
    if NewValue.Result == "OK" {
        if IsNumber(NewValue.Value) {
            if ItemName == "Change plain interval" {
